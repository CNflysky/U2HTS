...
static bool u2hts_usb_init_done = false;
...
static uint8_t u2hts_hid_desc[] = {
    ...
    HID_REPORT_ID(1) HID_USAGE(0x22),
    // 5 points
...
static void u2hts_irq_set(bool enable) {
  gpio_set_irq_enabled_with_callback(U2HTS_TP_INT, GPIO_IRQ_EDGE_FALL, enable,
                                     u2hts_irq_cb);
}
...
void tud_suspend_cb(bool remote_wakeup_en) {
  printf("device suspended, rmt_wake_en = %d\n", remote_wakeup_en);
}
