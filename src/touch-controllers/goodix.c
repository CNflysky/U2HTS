...
u2hts_touch_controller goodix = {.name = "Goodix",
                                 .i2c_addr = 0x5d,
...
static void goodix_read_tp_data(u2hts_options *opt, u2hts_hid_report *report) {
  ...
  for (uint8_t i = 0; i < tp_count; i++) {
    ...
    tp_data[i].x_coord =
        (tp_data[i].x_coord > opt->x_max) ? opt->x_max : tp_data[i].x_coord;
    tp_data[i].y_coord =
        (tp_data[i].y_coord > opt->y_max) ? opt->y_max : tp_data[i].y_coord;
    ...
    if (opt->x_invert) report->tp[i].x = U2HTS_LOGICAL_MAX - report->tp[i].x;

    if (opt->y_invert) report->tp[i].y = U2HTS_LOGICAL_MAX - report->tp[i].y;
